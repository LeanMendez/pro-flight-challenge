...
export const LogosList: React.FC = () => {
  ...
  const next = (): void => {
    const nextIndex =
      selectedIndexLogo < logos.length ? selectedIndexLogo + 1 : 1
    setSelectedIndexLogo(nextIndex)
  }

  const previous = (): void => {
    const previousIndex =
      selectedIndexLogo > 1 ? selectedIndexLogo - 1 : logos.length
        ...
        {logos.map((logo) => (
          <img
            className={`${styles.images} ${
              selectedIndexLogo === logo.id ? styles.active : styles.inactive
            }`}
            key={logo.id}
            src={logoBrand}
          />
